import { useTranslation } from '../hooks/useTranslation'  // BUG-07 FIX
import styles from './WhatsAppButton.module.css'

function WhatsAppIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 32 32" fill="none" aria-hidden="true">
      <circle cx="16" cy="16" r="16" fill="white" opacity="0.15"/>
...
export default function WhatsAppButton({ text }) {
  const { t } = useTranslation()  // BUG-07 FIX
  ...
  return (
    <button type="button" className={styles.btn} onClick={handleClick}>
      <WhatsAppIcon />
      <span>{t('btn_share_whatsapp')}</span>  {/* BUG-07 FIX: was hardcoded */}
    </button>
  )
}
